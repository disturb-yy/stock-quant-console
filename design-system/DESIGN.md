...
本文是量策的视觉样式基础：它定义默认的视觉方向、信息层级、布局节奏、组件模式与金融数据阅读方式。所有 UI 工作均应先阅读本文，再结合页面文档与 `MASTER.md` 实施。

本文包含对币安页面的研究事实；其中的外部品牌名称、商标、专有字体、文案、截图、图标与媒体仅用于说明观察到的样式逻辑，绝不是量策可直接使用的资产。`MASTER.md` 负责将这些逻辑转换为量策的 token、系统字体、可访问性要求与实现尺寸；两者冲突时，应更新映射，而不是在页面中临时复制来源产品。
...
- 本文中 BinanceNova 的“展示/正文无衬线”角色，映射为 `MASTER.md` 规定的中文系统无衬线字体栈；不加载或复制 BinanceNova 文件。
- 本文中 BinancePlex 的“可信金融数字”角色，映射为 `MASTER.md` 规定的系统等宽数字字体栈与 `tabular-nums`；不加载或复制 BinancePlex 文件。
- 本文的色彩、间距、圆角、表面与组件结论应由 `MASTER.md` 形成量策 token 后使用；不可把来源页面的品牌资产或未批准的私有 token 直接带入实现。
...
6. 数字始终使用 `MASTER.md` 的本地等宽数字映射，正文与导航始终使用本地无衬线映射；不得加载或复制 BinanceNova、BinancePlex 或其他来源产品字体。
...
<!-- 已采用的视觉样式基础：外部研究事实必须经过 MASTER 的本地映射后实施，不直接复制外部专有资产。 -->
