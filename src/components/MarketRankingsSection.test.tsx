...
describe('MarketRankingsSection', () => {
  ...
  it('shows ranking, stock fields, metric, observation date, source, and textual price direction', async () => {
    ...
    expect(screen.getByRole('link', { name: '300750.SZ' })).toHaveAttribute('href', '/stocks/300750.SZ')
    expect(screen.getByRole('link', { name: '宁德时代' })).toHaveAttribute('href', '/stocks/300750.SZ')
    ...
    const pageSizeInput = within(screen.getByLabelText('排行榜分页')).getByRole('textbox')
    expect(pageSizeInput.closest('.t-input__wrap')).not.toHaveClass('t-input--auto-width')
